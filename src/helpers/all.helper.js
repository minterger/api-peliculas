...
async function getInfo(uri) {
  try {
    ...
    const object = {};
    object.title = $(".m-b-5").text().trim();
    object.poster_img = $(".img-fluid.d-block.mx-auto.m-b-30").attr("src");
    object.descripcion = $(".text-large").html().trim();
    object.raiting = $(".font-size-18.text-info.text-semibold.ion-md-star")
      .text()
      .trim();
    object.descripcion = $(".text-large").html();
    object.ano = {
      text: $(".font-size-18.text-info.text-semibold").html().trim(),
      href: $(".p-v-20.p-r-15.text-center")
        .find("a")
        .attr("href")
        .replace(/\w{4,5}\W{3}(\w+\.?){1,3}/gi, ""),
    ...
    $(".container-fluid .card .card-body .p-v-20")
      .eq(1)
      .find("a")
      .each((i, el) => {
        const $el = $(el);
...
async function reqRepro(uri) {
  try {
    ...
    data.reproductores = $(".player script")
      .html()
